...
pub(crate) fn register_mailbox_routes(cfg: &mut web::ServiceConfig) {
    cfg.route("/api/emails", web::get().to(api_emails))
        ...
        .route("/api/templates/{id}", web::get().to(api_templates_get))
        ...
        .route(
            "/api/v1/drafts/scheduled/{id}",
            web::delete().to(api_scheduled_cancel),
        )
        .route(
            "/api/calendar/events",
...
mod tests {
    ...
    #[test]
    fn mailbox_routes_handler_api_templates_get() {
        let handler = "api_templates_get";
        assert_eq!(handler, "api_templates_get");
    }
