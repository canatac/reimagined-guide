...
- **Hermes gateway** (`POST /api/hermes/chat`, `POST /api/hermes/runs`, `GET /api/hermes/runs/{runId}`, `GET /api/hermes/runs/{runId}/events`) for secure server-to-server AI calls, including optional explicit `sessionId`/`sessionKey` overrides for cross-service thread continuity; events endpoint now streams SSE passthrough for live run updates.
