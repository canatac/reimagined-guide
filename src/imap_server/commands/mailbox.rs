...
mod tests {
    ...
    #[tokio::test]
    async fn handle_delete_returns_no_when_user_not_authenticated() {
        let mock_client = Box::new(MockDatabaseInterface::new());
        let logic = Arc::new(Logic::new_with_mock(mock_client));
        let mut server = ImapServer::new(logic);
        let sessions: Sessions = Arc::new(Mutex::new(HashMap::new()));
        let session_id = None;
        let command_parts = ["A1", "DELETE", "OldFolder"];

        let response = server
            .handle_delete("A1", &command_parts, &sessions, &session_id)
            .await;

        assert_eq!(response, "A1 NO DELETE failed: User not authenticated\r\n");
    }

    #[tokio::test]
    async fn handle_delete_returns_ok_for_authenticated_user() {
        let mut mock_client = Box::new(MockDatabaseInterface::new());
        mock_client
            .expect_delete_mailbox_for_user()
        ...
        sessions
            ...
            .insert("sess-1".to_string(), "testuser".to_string());
        let session_id = Some("sess-1".to_string());
        let command_parts = ["A2", "DELETE", "Projects"];

        let response = server
            .handle_delete("A2", &command_parts, &sessions, &session_id)
            .await;
